...
type PublicServiceStatus = { version?: number; available?: boolean; status?: string }
...
export async function isWebsiteServiceAvailable(): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${adminUrl}/api/public/service-status/${tenantId}`, { cache: "no-store", signal: AbortSignal.timeout(2000) })
...
export function isServiceGuardExcludedPath(pathname: string): boolean {
  return pathname.startsWith("/admin") || pathname.startsWith("/api") || pathname.startsWith("/_next") || pathname === "/favicon.ico" || pathname === "/service-expired" || /\.[a-z0-9]+$/i.test(pathname)
}
